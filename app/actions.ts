...
export async function createObjectParts(
  ...
  const { data: object } = await supabase.from("complex_objects").select("*").eq("user_id", userId).eq("quest_id", questId).single();

  if (object) {
    const { error } = await supabase.from("complex_objects").update({
      user_id: userId,
      quest_id: questId,
      ...data,
